...
- [~] **Techno Machine — Build the Rig** (tutorial) — `htmldoku/project-techno-machine.md`, drafted 2026-08-12. **Builder** shape, chosen by the owner over a player tutorial: Part 1 driver + autoconnect patch + restart order, Part 2 the eight chains + gain staging, Part 3 the sixteen post-fader inserts via `tools/build-techno-snapshot.py`, Part 4 commission the surface (five modes, page rings, pads-as-instrument, REC, SHIFT+GRID). All four parts `[draft]` — none walked on hardware yet. Playing technique stays in the manual. Daemon build is NOT repeated: it is a prerequisite on **Maschine MK2 Controller** Parts 1+3
  - Measured on the Pi while drafting, and both contradict the manual: the insert order on the wire is **strip → TAP Stereo Echo → TAP Reverberator → main** (the manual's §6 diagram has reverb first), and snapshot `016`'s saved strip levels are **not** the documented 0.19/0.80 — LEVEL writes the mixer, so the file carries the last jam's levels (kick 1.0, snare-openhat 0.19, bass 0.633, lead 0.58, pads 1.0, main 0.76). The tutorial teaches 0.19/0.80 as the design target and says why
  - FILTER mode's five drum columns are dead by design — `_set_voice_ctrl` finds no `VOICE_SYMBOLS` entry for `LS/LinuxSampler` and leaves the knob dead rather than moving something else. Read from code, not assumed

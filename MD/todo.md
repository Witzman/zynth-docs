...
- [~] **Techno Machine — 5 euclidean drums + 3 Turing voices, played from the Maschine (built 2026-08-10)**
  ...
  - [~] **Tutorial page DRAFTED 2026-08-12** — `htmldoku/project-techno-machine.md`, published as HTML with all four parts `[draft]`. **The drum-rig tutorial was dropped 2026-08-12** — the techno machine supersedes it
    - Owner chose the **builder** shape over a player tutorial: reproduce the rig from zero. The manual keeps playing technique
    - [ ] Walk Part 1 on hardware (driver files, autoconnect patch, restart order, "Loaded" not "Found")
    - [ ] Walk Part 2 on hardware (eight chains, MIDI 1-8, strips 0.19 / main 0.80, save into bank `000`)
    - [ ] Walk Part 3 on hardware (both inserts on one channel, dry 0.0 dB / wet −70 dB, `build-techno-snapshot.py`, reload+resave, `jack_lsp | grep -c TAP` → 64)
    - [ ] Walk Part 4 on hardware (five modes, page rings, FILTER's dead drum columns, pads-as-instrument, REC, SHIFT+GRID, gestures, health check)
    - **Two manual divergences found by measuring during the draft:** insert order on the wire is echo→reverb, not reverb→echo; and `016`'s saved levels are the last jam's, not 0.19/0.80
    - Unverified for the tutorial: the exact touchscreen label path for adding an Instrument chain's engine per channel was taken from `chain-management.md` (**Add Audio-FX processor** → **LV2 Plugin** is verified there); confirm the Add Chain wording on glass when walking Part 2

...
- [~] **Techno Machine pass two — SP1, SP5, SP2 and SP4 are DONE. SP3 is SHELVED by the owner. SP6, SP7 and SP8 remain**, all three from the owner's own ideas and none started
  ...
  - [~] **SP3 — SHELVED by the owner 2026-08-12.** No spec, no plan, nothing built; the owner has a different idea for the drum filter and will return to it. Everything learned is in `docs/superpowers/techno-machine/2026-08-12-sp3-filter-countertest.md` — read it before restarting, it saves a full measurement round
    - Decisions already taken and still standing: **per-channel** filters · **last in the chain**, after reverb and delay · surface **0-127** like the voices · migrate to a **new snapshot `017`** built from `016`, keeping `016` as fallback
    - **RezFilter's defaults are hostile:** `freq` 33 is *below the silence cliff*, `env_vcf` 70 and `lfo_vcf` 40 move the filter on their own, `max_freq` 75 caps the opening. All must be forced at build
    - The silence cliff is really between **32 and 30**, not 35; the 35 floor still stands because 33 is already 24 dB down
    - **Resonance clips at `res` 100** (peak 1.000 with RMS 0.2 noise; real drums clip sooner). The ceiling was never decided
    - **Calf Filter is the better plugin on paper** — Hz sweep, benign defaults, no LFO or envelope, `inertia` smoothing, gain compensation, 13 modes — but it **corrupts the heap in `lv2apply`** and cannot be verified with the tools on this Pi. Needs an in-rig test if anyone wants it
    - **TAL Filter is a blender: it runs, writes audio, and ignores every control port.** Byte-identical output at cutoff 1.0 and 0.0. A "does it run" check passes it. Only comparing settings exposes it
    - TalFilter2 has **no cutoff port at all** — an LFO effect, same naming trap as LSP's `surge_filter_stereo`
  - [x] **SP4 — channel type switching: BUILT, DEPLOYED, HARDWARE-VERIFIED (2026-08-12).** Spec `docs/superpowers/specs/2026-08-12-sp4-channel-type-design.md` · plan `…/plans/2026-08-12-sp4-channel-type.md` · findings `…/techno-machine/2026-08-12-sp4-test-findings.md`
    - **271 tests.** Six hardware checks, **zero defects found on hardware**
    - **SHIFT + GRID** switches the selected channel. The engine is NOT swapped: a drum kit gets the Turing generator and a synth gets euclid
    - **The register walks the KIT'S OWN NOTE LIST on a sampler**, not ROOT/SCALE — on an SFZ kit a note selects a sample, so scale quantising would land most steps on empty keys. Euclid on a synth is a **root pulse**, ROOT + the channel's OCTAVE
    - `kind_override` is `None` until switched, and **clears itself when you switch back to the chain's own kind** — so a channel is only overridden while it actually differs from its chain, and no stale choice can outlive an engine change
    - Per-kind state memory, stashed in the snapshot under `kinds` and `stash`. `div` and `beats` deliberately do NOT move: they are pattern time, not kind, and the groove must not jump on a switch
    - **Two snapshot defects caught in self-review, neither reached hardware:** `get_state` keyed the voice block on the CHANNELS table, so a switched drum channel would have lost its register and gate on save; and `set_state` restored the override without a matching state dict, which is a KeyError on the first repaint
    - **Confirmed by the owner:** hand-edited steps do not survive a switch, because switching rewrites the pattern from the generator — the same law as turning HITS. Per-kind memory covers parameters, never hand edits
    - **Not a defect:** a voice switched to drum may be silent, because `hits` can read back as 0. The tab draws dashed to say so; turn HITS up
  - [ ] **SP6 — the big encoder as the current page's master.** Owner's idea 2026-08-12, deliberately NOT folded into SP3: it is not filter-specific and serves every spread page that already exists — filter, mixer, chance, swing. Build it once, after SP3, and it lands on all of them.
    - **Trap, measured at G4:** the big encoder sends **8 units per detent** on CC 15 and wraps 120 → 0. The driver's wrap guard rejects anything from 8 units up, because real movement on the small encoders is 0-4 per report and their wraps are -38 to -40. **The big encoder sits exactly on the rejection threshold** and needs its own delta path, not `_enc_delta`. A naive wiring gives a dead knob — the same failure a too-tight wrap guard already caused once
    - Open for the owner: **additive or absolute?** Additive shifts all eight by the same amount and preserves the mix, but the channels pile up at the end stops. Absolute flattens the spread instantly. A filter sweep almost certainly wants additive
    - Open for the owner: **what does it do on a non-spread page?** Channel pages and generated plugin pages have no eight like values. Either it does nothing there — a knob that sometimes works — or it takes a fixed second meaning such as MASTER level. Without an answer it is a dead knob
    - Big encoder press is **CC 12**, also measured and unbound — free for a related gesture if one is wanted
  - [ ] **SP7 — spare chains, so a channel can become a real synth.** Owner's idea 2026-08-12, raised during SP4 and deliberately deferred: each channel carries both a sampler and a synth in the snapshot with one muted, so switching is instant AND changes the engine. Costs roughly 5% of a core idle for five more jalv hosts, plus RAM and a new snapshot. SP4 does the cheap half — the generator switches, the engine does not
  - [ ] **SP8 — a range-limited kit walk.** Confine SP4's Turing walk to part of the kit — only the hats, say — with the voice's RANGE column taking that role. Worth having once the full walk has been played for a while; the owner asked for it to be noted rather than built
...
- [~] **Techno Machine — 5 euclidean drums + 3 Turing voices, played from the Maschine (built 2026-08-10)**
  ...
  - [x] **Tutorial MOVED to its own repository 2026-08-13.** `htmldoku/project-techno-machine.md`, its HTML and its sidebar entry are deleted. The build guide is now **[Generative-Techno-ZynthianMaschine-MKII](https://github.com/Witzman/Generative-Techno-ZynthianMaschine-MKII)**, live at <https://witzman.github.io/Generative-Techno-ZynthianMaschine-MKII/> — seven sections plus two appendices, self-contained with the daemon, driver, 271 tests, system files, tools and snapshot `017-generative-techno`. Open items live in that repo now, not here
    - **Three measured facts that contradicted older docs**, all now recorded in the new repo: insert order on the wire is **echo → reverb**, not reverb → echo · `016`'s saved levels were the last jam's, not 0.19/0.80 · the kit directory holds **40** `.sfz` files, and `DYNOSAUR-808.sfz` is a **directory**, so `ls` on a `*.sfz` glob reports 61
    - **The zmip slot number is not fixed.** `maschine-jack-connect.sh` asks for `dev3_in`; the rig runs on `dev2_in` because Zynthian assigns the slot itself once `zynautoconnect` is patched. Check for exactly one route, never for a number — and not with `grep -A3`, which over-counts because `jack_lsp -c` interleaves connection blocks
    - FILTER mode's five drum columns are dead by design: `_set_voice_ctrl` finds no `VOICE_SYMBOLS` entry for `LS/LinuxSampler`, so law L4 applies
    - **Snapshot `017-generative-techno` is DONE and confirmed by ear 2026-08-13** — 124 BPM, four-on-the-floor A-D, channel E a drum kit walked by a Turing register (`kinds: {"4":"voice"}`), F frozen at LOCK, G full random, H one 8-step note per loop. Strips 0.19, main 0.774. All eight channels `gen`-owned. Shipped in the new repo
    - **A snapshot only stores the main strip if it is saved AFTER master is set.** The first `017` build had no `chan_16` entry at all, so it loaded leaving the main fader wherever it happened to be — silent-looking correctness with a wrong mix
    - **The old daemon fork `Witzman/MaschineMK2_linux` was deliberately NOT archived** (owner, 2026-08-13). The daemon is vendored in the new repo, but the fork stays writable — so if you commit daemon work, decide which repo is canonical first
